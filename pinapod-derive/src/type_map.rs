...
fn classify_vec_checked(ty: &Type) -> Result<TailField, TokenStream> {
	...
	if !(2..=3).contains(&arguments.len()) {
		return Err(compact_type_error(
			ty,
			"compact vectors require an element type, capacity, and optional 1, 2, 4, or 8-byte \
			 prefix",
		));
	}
	...
	if contains_dynamic_type(&elem) {
		return Err(compact_type_error(
			&elem,
			"unsupported dynamic compact vector element; supported compact fields are: \
			 `String<N>`, `Vec<T, N>` for fixed `T`, `Option<T>` for fixed `T`, \
			 `Option<String<N>>`, `Option<Vec<T, N>>` for fixed `T`, and `Vec<String<M>, N>`",
		));
	}
...
fn classify_option_checked(ty: &Type) -> Result<FieldKind, TokenStream> {
	...
	if is_vec_type(inner) {
		if vec_element(inner).is_some_and(is_string_type) {
			return Err(compact_type_error(
				inner,
				"unsupported dynamic nesting: `Option<Vec<String<_>, _>>` is not supported; \
				 supported compact fields are: `String<N>`, `Vec<T, N>` for fixed `T`, \
				 `Option<T>` for fixed `T`, `Option<String<N>>`, `Option<Vec<T, N>>` for fixed \
	...
	if contains_dynamic_type(inner) {
		return Err(compact_type_error(
			inner,
			"unsupported dynamic compact option payload; supported compact fields are: \
			 `String<N>`, `Vec<T, N>` for fixed `T`, `Option<T>` for fixed `T`, \
			 `Option<String<N>>`, `Option<Vec<T, N>>` for fixed `T`, and `Vec<String<M>, N>`",
		));
	}
