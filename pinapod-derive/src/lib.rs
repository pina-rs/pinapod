	...
	match crate_name("pinapod") {
		...
		Err(error) => {
			Err(syn::Error::new(
				...
				format!(
					"could not resolve the `pinapod` crate: {error}; reexports must set \
					 `#[pinapod(crate = path::to::pinapod)]`"
				),
			))
		}
	}
	...
	for variant in &data.variants {
		...
		for attr in &variant.attrs {
			...
			attr.parse_nested_meta(|meta| {
				...
				if !is_compact {
					return Err(meta.error(
						"`compact` payloads are only supported on variants of a compact PinaPod \
						 enum",
					));
				}
