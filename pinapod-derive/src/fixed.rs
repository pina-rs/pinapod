    ...
    let methods: Vec<TokenStream> = schema
		...
		.filter_map(|f| {
			...
			match classify_accessor(&f.ty, f.skip_accessor) {
				...
				AccessorKind::NativeViaFrom(native_ty) => {
					Some(quote! {
						#[inline(always)]
				...
				AccessorKind::Option(option) => {
					Some(match option {
						OptionAccessor::CopyDirect(inner) => {
							quote! {
								#[inline(always)]
						...
						OptionAccessor::NativeViaFrom(inner) => {
							quote! {
								#[inline(always)]
								pub fn #name(&self) -> ::core::option::Option<#inner> {
