...
CI installs the solvers directly from their GitHub releases: `z3` through the `cda-tum/setup-z3` action and `cvc5` through a dedicated `install-cvc5` action that pins the release archive by SHA-256, since cvc5 publishes six Linux x86_64 archives per version and a name-pattern match cannot be trusted to pick the self-contained static build. The Kani version is pinned alongside them. Proofs no longer run through the `devenv` environment, which saves roughly ten minutes per shard and keeps the devenv setup for local runs, where the same profile pins `cvc5` and `z3` for investigation.
