#![allow(
    ...
    missing_docs,
    reason = "adversarial tests construct raw layouts, preserve explicit upstream trait paths, \
              and are not a published surface"
)]
...
fn pod_vec_of_pod_option() {
    ...
    assert!(
        v.try_push(PodOption::<PodU64>::some(PodU64::from(42u64)))
            .is_ok()
    );
